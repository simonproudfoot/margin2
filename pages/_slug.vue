...
<div>
    <div v-if="hasFeaturedImage" class="bg-black absolute top-0 left-0 h-screen w-full flex">
        <nuxt-img :src="featuredImage" placeholder alt="" class="backImage w-full h-full object-cover opacity-80" />
    </div>
    <div v-if="hasFeaturedImage" class="absolute top-0 left-0 h-screen w-full flex justify-items-end ">
        <div class="flex items-end w-full">
            <div class="md:w-9/12 w-full md:push-1/4 py-12 px-12 justify-between z-10 bg-white font-bold max-w-[800px] h-fit bottom-0 right-0 flex">
                <h1 ref="header" class="pb- text-4xl" v-html="title"></h1>{{$store.state.hasFeaturedImage}}
...
export default {
    ...
    async asyncData({ params, $axios, store }) {
        ...
        store.commit('nav/SET_DARK', dark)
        return { post: post.data[0], title: title, content: post.data[0].content.rendered, excerpt: excerpt, slug: params.slug, hasFeaturedImage: post.data[0].featured_media, hasContactForm: post.data[0].acf.page_has_contact_form, featuredImage: featuredImage };
    },
    methods: {
        scrollToContent() {
